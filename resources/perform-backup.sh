...
for CURRENT_DATABASE in $(mysql -u $TARGET_DATABASE_USER -p$TARGET_DATABASE_PASSWORD -h $TARGET_DATABASE_HOST -P $TARGET_DATABASE_PORT -e 'SHOW DATABASES;' -s --skip-column-names); do

    if [ $CURRENT_DATABASE = "information_schema" ] || [ $CURRENT_DATABASE = "sys" ] || [ $CURRENT_DATABASE = "mysql" ] || [ $CURRENT_DATABASE = "performance_schema" ]
    then

      echo -e "Database backup skipped for $CURRENT_DATABASE."
      ...
      if sqloutput=$(mysqldump -u $TARGET_DATABASE_USER -h $TARGET_DATABASE_HOST -p$TARGET_DATABASE_PASSWORD -P $TARGET_DATABASE_PORT $BACKUP_CREATE_DATABASE_STATEMENT $CURRENT_DATABASE 2>&1 >/tmp/$DUMP); then
